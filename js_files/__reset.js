...
form.submit.addEventListener('click', () => {
    ...
    request.onload = () => {
        ...
        if(responseObject){
           ...
           sessionStorage.setItem('abc',responseObject.abc);
        }
    }
    const requestData = `email=${form.email.value}`;
    request.open('post', 'chatroom_reset.php');
...
form2.submit.addEventListener('click', () => { 
    ...
    request.onload = () => {
        ...
        if(responseObject1.ok){
            ...
            responseObject1.message.forEach((message) => {
              const li = document.createElement('li');
              li.textContent = message;
              form.message.appendChild(li);
            })
    ...
    const requestData = `email=${form2.email.value}&password=${form2.password.value}&confirm_password=${form2.confirm_password.value}`;
    request.open('post', 'chatroom_set_password.php');
    request.setRequestHeader('Content-type', 'application/x-www-form-urlencoded');
    request.send(requestData);
})	